...
class MoveToType(MiddleCommandType):
    ...
    def run(self, robot):
        pose = Transform.get_pose(self, self.EndPosition)
        id = self.CommandID
        if self.MoveStraight == True:
            robot.movel(id, pose, robot.transAccel, robot.transSpeed, 0)
        else:
            robot.movej_p(id, pose, robot.rotAccel, robot.rotSpeed, 0, 0)
