...
extern "C"{ // This is important, since avoid name mangling of the symbols, so the SAMYCore can find the correct chunck of compiled assembly code
	...
	*/
	void checkPalletSkill(UA_CRCL_PoseDataType const * const dummy){
			
		// Copy the parameter in local variable
		UA_CRCL_PoseDataType dummy_local = *dummy;
		...
		UA_EnableSensorParamsSetDataType enableSensor;
		enableSensor.name = UA_STRING("enableSensor");
		enableSensor.realTimeParameter = false;
		enableSensor.realTimeParameterNodeID = UA_NODEID_NUMERIC( 5, 0 );
		enableSensor.sensorID = UA_STRING("sensorID");
		enableSensor.sensorOptionSize = 0;
		enableSensor.sensorOption = NULL;

		UA_CRCLCommandsParamsSetsUnionDataType enableSensorUnion;
	    enableSensorUnion.switchField = UA_CRCLCOMMANDSPARAMSSETSUNIONDATATYPESWITCH_ENABLESENSORPARAMSSET;
	    enableSensorUnion.fields.enableSensorParamsSet = enableSensor;

		commands.push_back(enableSensorUnion);
