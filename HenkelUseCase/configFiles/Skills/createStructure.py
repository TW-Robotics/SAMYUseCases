...
class SkillCreator():
	def __init__(self, path, templates):
		self.foldernames = [ fodler.name for folder in os.scandir(".") if folder.is_dir() ]
		self.subfolderPaths = [ folder.path for folder in os.scandir(".") if folder.is_dir() ]
		self.templates = Templates()

	def processFolders(self):
		for subfolderPath in subfolderPaths:
			self.processSkill(subfolderPath)

	def parseSkillConfig(self, folderPath):
		fileStream = open(folderPath + '/SkillConfig.yaml', 'r')
		yamldata = yaml.safe_load(fileStream)
		data = yamldata["SAMYSkillConfig"]
		data["EntryFile"] = data["EntryFile"][:-3] # Remove .cpp from EntryFile name
		data["SkillNameLower"] = data["SkillName"][0].lower() + + SkillName[1:]
		#data["Foldername"] = 
		return data
	
	def processSkill(self, folderPath):
		skillData = parseSkillConfig(folderPath)

		output = renderCmake(skillData, folderPath)
		createFile("CMakeLists.txt", output)

		renderHeader(skillData, folderPath)
		createFile(skillData["EntryFile"] + ".cpp", output)

		renderCpp(skillData, folderPath)
		createFile(skillData["EntrFile"] + ".h", output)

		createBuildFolder(folderPath)

	def renderCmake(self, data, path):
		template = self.templates.cppTemplate
		return template.render(data)

	def renderHeader(self, data, path):
		template = self.templates.headerTemplate
		return template.render(data)

	def renderCpp(self, data, path):
		template = self.templates.cppTemplate
		return template.render(data)
...
set (LIBRARY_NAME "{{SkillNameLower}}")
...
target_sources(${LIBRARY_NAME}
  PUBLIC
  ${CMAKE_CURRENT_SOURCE_DIR}/{{EntryFile}}.cpp
)
...
target_link_libraries( ${LIBRARY_NAME} "${CMAKE_SOURCE_DIR}/../../../build/SAMYCore_Library.so.2.0")"""
		...
		self.headerTemplate = """#pragma once 
#include <cstdint>
#include <dylib.h>
...
extern "C" {
	...
	void {{ EntryFunction }}({% for x in Parameters -%}{% if x.DataType[:2] != 'UA' %}UA_{% endif %}{{x.DataType}} const * const {{x.Name}}{% if not loop.last %},
	        {% endif %}{% endfor %});
    
}"""

		self.cppTemplate = """#include <{{ EntryFile }}.h>
#include <skillsScriptingAPI.h>
#include <functional>
#include <iostream>
...
extern "C"{ // This is important, since avoid name mangling of the symbols, so the SAMYCore can find the correct chunck of compiled assembly code
	...
	*/
	void {{ EntryFunction }}({% for x in Parameters -%}{% if x.DataType[:2] != 'UA' %}UA_{% endif %}{{x.DataType}} const * const {{x.Name}}{% if not loop.last %}, {% endif %}{% endfor %}){
